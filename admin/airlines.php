				...
				<div class="card">
					...
					<div class="card-body">
							...
							<div class="form-group">
								<label class="control-label">الخطوط الجويه</label>
								<textarea name="airlines" id="" cols="30" rows="2" class="form-control"></textarea>
							</div>
							<div class="form-group">
								<label for="" class="control-label">الشعار</label>
						...
						<table class="table table-bordered table-hover">
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-center">
										<button class="btn btn-sm btn-primary edit_airline" type="button" data-id="<?php echo $row['id'] ?>" data-airlines="<?php echo $row['airlines'] ?>" data-logo_path="<?php echo $row['logo_path'] ?>" >تعديل</button>
...
<script>
	...
	$('#manage-airlines').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("تم أضافه البيانات بنجاح",'success')
					setTimeout(function(){
						location.reload()
					},1500)
	...
	$('.delete_airline').click(function(){
		_conf("هل انت متأكد من انك تريد الحذف ؟","delete_airline",[$(this).attr('data-id')])
	})
