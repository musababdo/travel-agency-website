...
Class Action {
	...
	function login2(){
		extract($_POST);
		$qry = $this->db->query("SELECT * FROM login where email = '".$email."' and password = '".$password."' ");
		if($qry->num_rows > 0){
			foreach ($qry->fetch_array() as $key => $value) {
				if($key != 'passwors' && !is_numeric($key))
					$_SESSION['login_'.$key] = $value;
	...
	function signup(){
		...
		$data .= ", email = '$email' ";
		$data .= ", password = '$password' ";
		$chk = $this->db->query("SELECT * FROM login where email = '$email' ")->num_rows;
		if($chk > 0){
			return 2;
			exit;
		}
	...
	function update_booked(){
		extract($_POST);
			$data = "  name = '$name' ";
			$data .= " , phone = '$phone' ";
			$data .= " , desination_city = '$desination_city' ";
