...
<head>
  ...
  <title>وكاله الأسكله للسفر والسياحه لوحه التحكم</title>
  			...
  			<div class="card col-md-8">
  				<div class="card-body">
  					<form id="login-form" >
  						...
  						<center><button class="btn-sm btn-block btn-wave col-md-4 btn-primary">تسجيل دخول</button></center>
  					</form>
  				</div>
  			</div>
  		</div>
		...
		$('#login-form button[type="button"]').removeAttr('disabled').html('Login');
			...
			success:function(resp){
				if(resp == 1){
					...
					$('#login-form').prepend('<div class="alert alert-danger">اسم المستخدم او كلمه المرور غير صحيحه</div>')
					$('#login-form button[type="button"]').removeAttr('disabled').html('Login');
				}
			}
		})
