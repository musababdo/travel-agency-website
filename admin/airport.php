				...
				<div class="card">
					...
					<div class="card-body">
							...
							<div class="form-group">
								<label class="control-label">أسم المطار</label>
								<input name="airport" id="" cols="30" rows="2" class="form-control"/>
							</div>
							<div class="form-group">
								<label class="control-label">موقع المطار</label>
					...
					<div class="card-footer">
						<div class="row">
							<div class="col-md-12">
								...
								<button class="btn btn-sm btn-default col-sm-3" type="button" onclick="_reset()"> الغاء</button>
							</div>
						</div>
					</div>
				</div>
						...
						<table class="table table-bordered table-hover">
							<thead>
								<tr>
									...
									<th class="text-center">أسم المطار</th>
									<th class="text-center">موقع المطار</th>
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-center">
										<button class="btn btn-sm btn-primary edit_airline" type="button" data-id="<?php echo $row['id'] ?>" data-airport="<?php echo $row['airport'] ?>" data-location="<?php echo $row['location'] ?>" >تعديل</button>
										<button class="btn btn-sm btn-danger delete_airline" type="button" data-id="<?php echo $row['id'] ?>">حذف</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					...
					alert_toast("تم تعديل البيانات بنجاح",'success')
					setTimeout(function(){
						location.reload()
					},1500)
	...
	$('.delete_airline').click(function(){
		_conf("هل انت واثق من انك تريد الحذف ؟","delete_airline",[$(this).attr('data-id')])
	})
